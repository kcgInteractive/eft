...
class PortfolioList implements Lib\ShortcodeInterface {
    ...
    */
	public function getHolderClasses($params){
		...
		$classes[] = !empty($params['type']) ? 'edgtf-pl-'.$params['type'] : 'edgtf-pl-gallery';
		$classes[] = !empty($params['space_between_items']) ? 'edgtf-pl-'.$params['space_between_items'].'-space' : 'edgtf-pl-normal-space';
		...
		if(!empty($params['item_style'])){
			$classes[] = 'edgtf-pl-'.$params['item_style'];
		}
		
		if($params['enable_fixed_proportions'] === 'yes') {
			$classes[] = 'edgtf-pl-images-fixed';
		}
		
		if($params['enable_title'] === 'no' && $params['enable_category'] === 'no' && $params['enable_excerpt'] === 'no') {
			$classes[] = 'edgtf-pl-no-content';
		}
		
		if(!empty($params['pagination_type'])){
			$classes[] = 'edgtf-pl-pag-'.$params['pagination_type'];
		}
		
		if($params['filter'] === 'yes'){
			$classes[] = 'edgtf-pl-has-filter';
		}
		
		if($params['enable_article_animation'] === 'yes'){
			$classes[] = 'edgtf-pl-has-animation';
		}
		
		if(!empty($params['navigation_skin'])) {
			$classes[] = 'edgtf-nav-'.$params['navigation_skin'].'-skin';
		}
		
		if(!empty($params['pagination_skin'])) {
			$classes[] = 'edgtf-pag-'.$params['pagination_skin'].'-skin';
		}
		
		if(!empty($params['pagination_position'])) {
			$classes[] = 'edgtf-pag-'.$params['pagination_position'];
		}
		...
		if($params['type'] == 'masonry' && $params['enable_fixed_proportions'] === 'yes') {
			...
			switch ($fixed_image_size) {
				...
				case 'large-width-height':
					$thumb_size = 'fluid_edge_image_huge';
					break;
				default :
					$thumb_size = 'full';
					break;
