...
class PortfolioProjectInfo implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
        if(function_exists('vc_map')) {
	        vc_map( array(
			        'name'                      => esc_html__( 'Edge Portfolio Project Info', 'edgtf-core' ),
			        'base'                      => $this->getBase(),
			        ...
			        'params'                    => array(
				        array(
					        ...
					        'heading'    => esc_html__( 'Selected Project', 'edgtf-core' ),
					        'settings'   => array(
						        'sortable'      => true,
						        'unique_values' => true
					        ),
					        'description' => esc_html__( 'If you left this field empty then project ID will be of the current page', 'edgtf-core' )
				        ),
				        array(
					        'type'        => 'dropdown',
					        'param_name'  => 'project_info_type',
					        'heading'     => esc_html__( 'Project Info Type', 'edgtf-core' ),
					        'value'       => array(
						        esc_html__( 'Title', 'edgtf-core' )    => 'title',
						        esc_html__( 'Category', 'edgtf-core' ) => 'category',
						        esc_html__( 'Tag', 'edgtf-core' )      => 'tag',
						        esc_html__( 'Author', 'edgtf-core' )   => 'author',
				        ...
				        array(
					        ...
					        'heading'     => esc_html__( 'Project Title Tag', 'edgtf-core' ),
					        'value'       => array_flip(fluid_edge_get_title_tag(true, array('p' => 'p'))),
					        'description' => esc_html__( 'Set title tag for project title element', 'edgtf-core' ),
					        'dependency'  => array( 'element' => 'project_info_type', 'value' => array( 'title' ) )
				        ),
				        array(
					        'type'        => 'textfield',
					        'param_name'  => 'project_info_title',
					        'heading'     => esc_html__( 'Project Info Label', 'edgtf-core' ),
					        'description' => esc_html__( 'Add project info label before project info element/s', 'edgtf-core' )
				        ),
				        array(
					        'type'       => 'dropdown',
					        'param_name' => 'project_info_title_tag',
					        'heading'    => esc_html__( 'Project Info Label Tag', 'edgtf-core' ),
					        'value'      => array_flip(fluid_edge_get_title_tag(true, array('p' => 'p'))),
	...
	public function getItemTitleHtml($params){
		...
		if(!empty($title)) {
			$html = '<'.esc_attr($project_info_title_type_tag).' itemprop="name" class="edgtf-ppi-title entry-title">';
