<?php
namespace EdgeCore\CPT\Shortcodes\GalleryBlocks;
...
class GalleryBlocks implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Edge Gallery Blocks', 'edgtf-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'description' => esc_html__( 'Select images from media library. The first image you upload will be set as the featured image if you set Featured Image Size.', 'edgtf-core' )
						),
						array(
							'type'        => 'textfield',
							'param_name'  => 'featured_image_size',
							'heading'     => esc_html__( 'Featured Image Size', 'edgtf-core' ),
	...
	public function render($atts, $content = null) {
		...
		$html = edgtf_core_get_shortcode_module_template_part('templates/gallery-blocks', 'gallery-blocks', '', $params);
	...
	private function getHolderClasses($params){
		...
		if(!empty($params['space_between_columns'])){
			$holderClasses .= ' edgtf-'.$params['space_between_columns'].'-space';
		}
